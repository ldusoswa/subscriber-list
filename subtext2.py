...
class Member:
    def __init__(self, name, joined):
        self.name = name
        self.joined = joined  # Expecting a date object

    def __str__(self):
        return f"{self.name} joined on {self.joined}"
...
with open(twitchSubsFile, 'r') as csv_file:
    ...
    for row in sortedlist:
         allMembers.append(Member(row[0], row[3]))
...
with open(patroenSubsFile, 'r') as csv_file:
    ...
    for row in sortedlist:
        allMembers.append(Member(row[0], row[8]))
#             # print(row[10] + ' - ' + row[0])
...
with open(youtubeSubsFile, 'r', encoding='utf-8') as csv_file:
    ...
    for row in sortedlist:
        allMembers.append(Member(row[0], row[4]))
#         if row[2] == "Pit Crew":
#             if float(row[4]) > 3:
...
# print(f'TWITCH (€4.99/mo)\t\t{", ".join(twitchCombined)}')
